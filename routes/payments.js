...
router.post("/create-checkout", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT o.*, c.name AS customer_name, c.email, c.stripe_customer_id, c.stripe_mandate_id, c.direct_debit_active
    ...
    const paidTotal = Number(summary[0]?.paid_total || 0);
    const refundedTotal = Math.abs(Number(summary[0]?.refunded_total || 0));
    ...
    // 4️⃣ Determine mode + payment type
...
router.post("/refund", async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [payment.order_id, payment.customer_id, -Math.abs(amount), payment.method, refund.id]
    );

    console.log(`💸 Refund recorded for order ${payment.order_id} (£${amount})`);
